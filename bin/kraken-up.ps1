...
inf "Building kraken container:`n  'docker build -t samsung_cnct/kraken:$clustername -f '$krakenRoot/bin/build/Dockerfile' '$krakenRoot' '"
Invoke-Expression "docker build -t samsung_cnct/kraken:$clustername -f '$krakenRoot/bin/build/Dockerfile' '$krakenRoot'"

# run cluster up
$command =  "docker run -d --name $kraken_container_name -v /var/run:/ansible --volumes-from kraken_data samsung_cnct/kraken:$clustername bash -c " +
