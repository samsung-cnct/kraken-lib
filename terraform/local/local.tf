...
resource "execute_command" "command" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '${path.module}/rendered/ansible.inventory' --connection=local ${path.module}/../../ansible/generate_local_kubeconfig.yaml"
  }

  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '${path.module}/rendered/ansible.inventory' --connection=local ${path.module}/../../ansible/generate_local_ssh_config.yaml"
  }
}
