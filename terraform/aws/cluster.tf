...
resource "template_file" "ansible_inventory" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${module.aws.template} ${path.module}/../../ansible/setup-python.yaml"
  }

  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${module.aws.template} ${path.module}/../../ansible/setup-kubernetes.yaml"
  }
}
