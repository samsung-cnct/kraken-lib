...
def translate_config(config_data):
  try:
    all_data = copy.deepcopy(expand_envs(config_data))
    return all_data
  except Exception, e:
    raise errors.AnsibleFilterError(
            'translate_config plugin error: {0}, config_data={1}'.format(
              str(e),
              str(config_data)))

def expand_envs(obj):
  if isinstance(obj, dict):
    # backwards compatibilty for etcdConfig and etcdConfigs
    if 'nodePools' in obj:
      nodePools = obj['nodePools']
      for pool in nodePools:
        if 'etcdConfig' in pool:
          pool['etcdConfigs'] = [ pool['etcdConfig'] ]
          del pool['etcdConfig']
