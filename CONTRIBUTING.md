...
Before contributing code, please familiarize yourself with the [kraken codebase](https://github.com/samsung-cnct/kraken) and our general guidelines below:
...
You can access the recommended kraken-lib developer tool in the /hack directory. Called `dockerdev`, it pulls down a [kraken-tools](https://github.com/samsung-cnct/kraken-tools) image and lands the user into a Bash prompt with a number of useful host directories mounted into the container including the local kraken-lib checkout, `~/.ssh`, `~/.aws` and more depending on the config file used.
